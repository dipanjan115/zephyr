...
static void common_configure(uint16_t addr)
{
	...
	for (int i = 0; i < ARRAY_SIZE(model_ids); i++) {
		...
		/* FOR GROUPCAST*/
		if (addr == UNICAST_ADDR1 || addr == UNICAST_ADDR4 || addr == UNICAST_ADDR5) {
			err = bt_mesh_cfg_cli_mod_sub_add(0, addr, addr, GROUP_ADDR1, model_ids[i],
							  &status);
			if (err || status) {
				FAIL("Model %#4x subscribe failed (err %d, status %u)",
